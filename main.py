...
# Features: Premium UI + Logic Gates + Case Watch + Secure Access + Citation Verification
...
# CITATION BADGE CSS
# ---------------------------------------------------------
# CSS for verification badges on citations and precedent cards
CITATION_BADGE_CSS = """
<style>
.citation-badge {
...
# Inject citation badge CSS early
...
# Initialize citation recommender for precedent suggestions
...
try:
    ...
    # Citation modules not available - precedent suggestions disabled
    pass
except Exception as e:
    # Other initialization error - precedent suggestions disabled
    pass
...
def display_precedents(recommendations: list):
    ...
    for rec in recommendations:
        ...
        <div class="precedent-card {badge_class}">
            ...
            <p style="margin-top: 8px; font-size: 0.9em;">{snippet}</p>
        </div>
        """
...
with tab1:
    ...
    if user_input := st.chat_input("Ex: Can I file a suit without mediation?"):
        ...
        # Show relevant precedents if citation recommender is available
        if citation_recommender:
            with st.expander("Show Relevant Precedents"):
                with st.spinner("Finding relevant precedents..."):
                    try:
                        precedents = citation_recommender.recommend_precedents(
                            legal_issue=user_input,
                            filing_court="supreme_court",
                            top_k=5
                        )
                        display_precedents([p.model_dump() for p in precedents])
                    except Exception as e:
                        st.warning(f"Could not retrieve precedents: {str(e)}")
